...
export default function AdminPage() {
  ...
  const fetchPending = async () => {
    ...
    try {
      const res = await fetch("http://localhost:5000/api/artworks/pending", {
  ...
  const fetchAllArtworks = async () => {
    ...
    if (!token) return;
    try {
      const res = await fetch("http://localhost:5000/api/artworks/admin/all", {
        headers: { "x-auth-token": token }
      });
      if (res.ok) {
        const data = await res.json();
  ...
  const handleStatusUpdate = async (id, newStatus) => {
    const token = localStorage.getItem('token');
    try {
      const res = await fetch(`http://localhost:5000/api/artworks/${id}/status`, {
        method: "PUT",
        headers: { 
